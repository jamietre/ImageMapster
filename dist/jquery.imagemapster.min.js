...
b.Method.prototype.go=function(){var g,f,c,h,k,l=this.input,d=[],j=this,e=j.args||[];
h=l.length;
for(g=0;
g<h;
g++){f=a.mapster.getMapData(l[g]);
if(f){if(b.queueCommand(f,this.input,this.name,e)){if(this.first){k=""
}continue
}c=f.getData(l[g].nodeName==="AREA"?l[g]:this.key);
if(c){if(a.inArray(c,d)<0){d.push(c)
}}else{k=this.func_map.apply(f,e||[])
}if(this.first||typeof k!=="undefined"){break
}}}a(d).each(function(){k=j.func_area.apply(this,e)
});
...
a.each(e.areas(),function(k,j){var l=this.effectiveOptions(h);
...
h.highlight(!f.options.highlight);
...
b.MapArea.prototype.effectiveOptions=function(h){var f,e,g=this.owner,j=d.updateProps({},g.area_options);
for(f=this.keys.length-1;
f>=0;
f--){e=g.getDataForKey(this.keys[f]);
d.updateProps(j,e.options,e.options["render_"+h],{alt_image:this.owner.altImage(h)})
}return j
}
}(jQuery));
