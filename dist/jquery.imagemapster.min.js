...
}}if(f.activeAreaEvent){window.clearTimeout(f.activeAreaEvent);}if(l<0){return;}if(i.owner.resizing||l){f.activeAreaEvent=window.setTimeout((function(){return function(m){h(0,m,j);
...
if(!i||i.isNotRendered()||i.owner.resizing){return;}l=i.effectiveOptions();f.inArea=true;
if(!a.mapster.hasCanvas){this.blur();}if(f.currentAreaId===i.areaId){return;}if(f.highlightId!==i.areaId){f.clearEffects();
...
this.currentAreaId=-1;this._tooltip_events=[];this.scaleInfo=null;this.inArea=false;
