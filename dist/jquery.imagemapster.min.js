...
if(e===""){c.splice(f,1);}else{c[f]=d?d(e):e;}}return c;},setOpacity:function(c,f){if(!a.mapster.hasCanvas){var d=a(c);
...
}},fader:(function(){var c={},e=0,d=function(g,k,h,f){var i,j,l=a.mapster.utils;if(typeof g==="number"){j=c[g];
if(!j){return;}}else{i=l.indexOfProp(c,null,g);if(i){delete c[i];}c[++e]=j=g;g=e;
}h=h||1;k=(k+(h/10)>h-0.01)?h:k+(h/10);l.setOpacity(j,k);if(k<h){setTimeout(function(){d(g,k,h,f);
...
this.options=d.updateProps({},b.defaults,e);this.bindTries=this.options.configTimeout/50;
...
a(f).attr("src",k);}else{m(f);}};c.isImageLoaded=function(f){var e,g=this;if(g.imageStatus[f]){return true;
}e=g.images[f];if(typeof e.complete!=="undefined"&&!e.complete){return false;}g.imageStatus[f]=!!d.imgWidth(e);
return g.imageStatus[f];};c.bindImages=function(g,e){var h,k=this,j=true,l=k.options,m=function(){k.bindImages.call(k,false,e);
},f=function(i){window.clearTimeout(k.imgTimeout);k.imageLoadError(i);};if(g){k.complete=false;
k.triesLeft=k.bindTries;k.imagesLoaded=false;if(k.images.length>2){k.images=k.images.slice(0,2);
k.imageSources=k.imageSources.slice(0,2);k.imageStatus=k.imageStatus.slice(0,2);k.altImagesXref={};
}k.altImagesXref={};if(k.images.length===0){k.addImage(k.image);k.addImage(null,k.image.src);
}if(a.mapster.hasCanvas){k.addImage(null,l.render_highlight.altImage||l.altImage,"highlight");
k.addImage(null,l.render_select.altImage||l.altImage,"select");}}if(k.imagesLoaded){return;
}h=k.images.length;while(h-->0){if(!k.isImageLoaded(h)){j=false;break;}}k.imagesLoaded=j;
if(k.isReadyToBind()){if(e){e();}else{k.initialize();}return;}if(k.triesLeft-->0){this.imgTimeout=window.setTimeout(m,50);
}else{f();}};c.imageLoadError=function(f){var g=f?"The image "+f.target.src+" failed to load.":"The images never seemed to finish loading. This could be because of interference from a plugin.";
...
}});};c.initialize=function(){var j,e,m,r,n,f,g,h,p,q,o,k=this,l=k.options;if(k.complete){return;
}h=a(k.image);n=h.parent().attr("id");if(n&&n.length>=12&&n.substring(0,12)==="mapster_wrap"){r=h.parent();
r.attr("id",k.wrapId());}else{r=a('<div id="'+k.wrapId()+'"></div>');if(l.wrapClass){if(l.wrapClass===true){r.addClass(h.attr("class"));
}else{r.addClass(l.wrapClass);}}}k.wrapper=r;e=k.graphics.createVisibleCanvas(k.image);
m=k.graphics.createVisibleCanvas(k.image);k.base_canvas=e;k.overlay_canvas=m;k.scaleInfo=o=d.scaleMap(k.images[0],k.images[1],l.scaleMap);
j=a(k.images[0]).clone().addClass("mapster_el").attr({id:null,usemap:null});a.each(["width","height","padding","border","margin"],function(t,s){j.css(s,h.css(s));
});k.images[1]=j[0];k.buildDataset();f={display:"block",position:"relative",padding:0,width:o.width,height:o.height};
if(l.wrapCss){a.extend(f,l.wrapCss);}if(h.parent()[0]!==k.wrapper[0]){h.before(k.wrapper);
}r.css(f);a(k.images.slice(2)).hide();for(g=1;g<k.images.length;g++){r.append(k.images[g]);
}r.append(e).append(m).append(h.css(b.canvas_style));d.setOpacity(k.images[0],0);
a(k.images[1]).show();d.setOpacity(k.images[1],1);if(l.isSelectable&&l.onGetList){q=k.data.slice(0);
if(l.sortList){if(l.sortList==="desc"){p=function(i,s){return i===s?0:(i>s?-1:1);
};}else{p=function(i,s){return i===s?0:(i<s?-1:1);};}q.sort(function(i,s){i=i.value;
s=s.value;return p(i,s);});}k.options.boundList=l.onGetList.call(k.image,q);}k.complete=true;
...
}k=m?"":g.getAttribute(v.mapKey);if(w){s=u.mapAreas[e.data("mapster")-1];s.configure(k);
}else{s=new b.MapArea(u,g,k);u.mapAreas.push(s);}r=s.keys;for(p=r.length-1;p>=0;p--){q=r[p];
if(v.mapValue){n=e.attr(v.mapValue);}if(m){h=f(u.data.length,n);l=u.data[h];l.key=q=h.toString();
}else{h=u._xref[q];if(h>=0){l=u.data[h];if(n&&!u.data[h].value){l.value=n;}}else{h=f(q,n);
l=u.data[h];l.isPrimary=p===0;}}s.areaDataXref.push(h);l.areasXref.push(t);}o=e.attr("href");
if(o&&o!=="#"&&!l.href){l.href=o;}if(!s.nohref){e.bind("mouseover.mapster",u.mouseover).bind("mouseout.mapster",u.mouseout).bind("click.mapster",u.click).bind("mousedown.mapster",u.mousedown);
}if(m){e.attr("data-mapster-key",q);s.keys=[q];}e.data("mapster",t+1);}u.setAreaOptions(v.areas);
u.redrawSelections();};c.processCommandQueue=function(){var e,f=this;while(!f.currentAction&&f.commands.length){e=f.commands[0];
...
}return j;}return this.getScaleInfo(h(f),h(e));};b.MapData.prototype.resize=function(n,l,f){var j,p,s,i,e,o={},m,g,k=this;
function r(t,v,u){if(a.mapster.hasCanvas){t.width=v;t.height=u;}else{a(t).width(v);
a(t).height(u);}}function h(){r(k.overlay_canvas,s,i);if(o.highlight&&j>=0){var t=k.data[j];
t.tempOptions={fade:false};k.getDataForKey(t.key).highlight();t.tempOptions=null;
}r(k.base_canvas,s,i);k.redrawSelections();k.currentAction="";if(a.isFunction(o.callback)){o.callback();
}k.processCommandQueue();}function q(){a(k.image).css(m);k.scaleInfo=d.getScaleInfo({width:k.scaleInfo.realWidth,height:k.scaleInfo.realHeight},{width:s,height:i});
a.each(k.data,function(u,t){a.each(t.areas(),function(w,v){v.resize();});});}if(typeof n==="object"){o=n;
}else{o.width=n;o.height=l;o.duration=f||0;}s=o.width;i=o.height;e=o.duration;if(k.scaleInfo.width===s&&k.scaleInfo.height===i){return;
}j=k.highlightId;if(!s){p=i/k.scaleInfo.realHeight;s=Math.round(k.scaleInfo.realWidth*p);
}if(!i){p=s/k.scaleInfo.realWidth;i=Math.round(k.scaleInfo.realHeight*p);}m={width:String(s)+"px",height:String(i)+"px"};
if(!a.mapster.hasCanvas){a(k.base_canvas).children().remove();}g=a(k.wrapper).find(".mapster_el");
if(e){k.currentAction="resizing";g.each(function(u,t){a(t).animate(m,{duration:e,complete:u===0?h:null,easing:"linear"});
});a(k.wrapper).animate({scrollLeft:o.scrollLeft||0,scrollTop:o.scrollTop||0},{duration:e,easing:"linear"});
q();}else{g.css(m);h();q();}};b.MapArea=d.subclass(b.MapArea,function(){this.base.init();
