...
d.children().add(d).each(function(){this.style.filter="Alpha(opacity="+String(f*100)+");";
});}else{c.style.opacity=f;}},updateProps:function(c,d){var e,f=c||{},g=a.isEmptyObject(f)?d:c;
e=[];a.each(g,function(h){e.push(h);});a.each(Array.prototype.slice.call(arguments,1),function(h,j){a.each(j||{},function(k,l){if(!e||a.inArray(k,e)>=0){var i=j[k];
if(typeof i!=="undefined"){if(a.isPlainObject(i)){f[k]=a.extend(f[k]||{},i);}else{if(i&&i.constructor===Array){f[k]=i.slice(0);
}else{f[k]=j[k];}}}}});});return f;},isElement:function(c){return(typeof HTMLElement==="object"?c instanceof HTMLElement:c&&typeof c==="object"&&c.nodeType===1&&typeof c.nodeName==="string");
...
}var c,d,h=a(),f=e.split(",");g.boundList.each(function(){for(c=0;c<f.length;c++){d=f[c];
if(a(this).is("["+g.listKey+'="'+d+'"]')){h=h.add(this);}}});return h;},setBoundListProperties:function(c,e,d){e.each(function(){if(c.listSelectedClass){if(d){a(this).addClass(c.listSelectedClass);
}else{a(this).removeClass(c.listSelectedClass);}}if(c.listSelectedAttribute){a(this).attr(c.listSelectedAttribute,d);
...
}}}a(d).each(function(){j=h.func_area.apply(this,h.args);});if(typeof j!=="undefined"){return j;
...
break;default:t.toggleSelection();break;}}}k=i.isBool(r)?r:true;this.each(function(){var t;
o=d.getMapData(this);if(!o){return true;}m="";if(a(this).is("img")){if(d.queueCommand(o,a(this),"set",[q,l,k])){return true;
}if(l instanceof Array){if(l.length){m=l.join(",");}}else{m=l;}if(m){a.each(i.split(m),function(v,u){s(o.getDataForKey(u.toString()));
});if(!q){o.removeSelectionFinish();}}}else{p=a(this).parent()[0];if(n&&p!==n){o=d.getMapData(this);
if(!o){return true;}n=p;}n=p;if(d.queueCommand(o,a(this),"set",[q,l,k])){return true;
}t=o.getDataForArea(this);if(a.inArray(t,j)<0){j.push(t);m+=m===""?"":","+t.key;}}});
...
a.each(this.data,function(){this.options={};});}g(this,j);this.setAreaOptions(j.areas||{});
...
},null,{name:"snapshot"})).go();};e.state=function(){var j,k=null;a(this).each(function(){if(this.nodeName==="IMG"){j=d.getMapData(this);
if(j){k=j.state();}return false;}});return k;};e.bind=function(j){var k=i.updateProps({},d.defaults,j);
return this.each(function(){var l,m,o,n;l=a(this);l.css("border",0);n=d.getMapData(this);
if(n){e.unbind.apply(l);if(!n.complete){l.bind();return true;}n=null;}o=this.getAttribute("usemap");
m=o&&a('map[name="'+o.substr(1)+'"]');if(!(l.is("img")&&o&&m.size()>0)){return true;
}if(!n){n=new d.MapData(this,k);n.index=c(n);n.map=m;n.addImage(this);n.addImage(null,this.src);
if(a.mapster.hasCanvas){n.addImage(null,k.render_highlight.altImage||k.altImage,"highlight");
n.addImage(null,k.render_select.altImage||k.altImage,"select");}n.bindImages();}});
...
});}a(window).bind("load",function(){d.windowLoaded=true;a(d.map_cache).each(function(){if(!this.complete&&this.isReadyToBind()){this.initialize();
...
};c._addShapeGroupImpl=function(e,h){var g=this,f=g.map_data;a.each(e.areas(),function(k,j){var l=j.effectiveRenderOptions(h);
l.isMask=l.isMask||(j.nohref&&f.options.noHrefIsMask);g.addShape(j,l);});};c.addShapeGroup=function(e,j){var i=this,g,k,f,h=this.map_data,l=e.effectiveRenderOptions(j);
...
i.highlight(!l.highlight);if(f.options.showToolTip){a.each(j,function(){if(this.effectiveOptions().toolTip){this.showTooltip();
...
h(m.mouseoutDelay,i,function(){f.clearEffects();if(a.isFunction(m.onMouseout)){m.onMouseout.call(this,{e:j,options:m,key:k,selected:i.isSelected()});
}});};this.clearEffects=function(i){var j=f.options;f.ensureNoHighlight();if(j.toolTipClose&&a.inArray("area-mouseout",j.toolTipClose)>=0&&this.activeToolTip){f.cancelClear=false;
window.setTimeout(function(){if(!f.cancelClear){f.clearTooltip();}f.cancelClear=false;
},50);}};this.click=function(l){var q,m,n,o,j,r=this,i=f.getDataForArea(this),p=f.options;
l.preventDefault();if(!i||i.owner.resizing){return;}if(!a.mapster.hasCanvas){this.blur();
}p=f.options;function k(s){var t;j=(s.isSelectable()&&(s.isDeselectable()||!s.isSelected()));
if(j){o=!s.isSelected();}else{o=s.isSelected();}n=b.getBoundList(p,s.key);if(a.isFunction(p.onClick)){if(false===p.onClick.call(r,{e:l,listTarget:n,key:s.key,selected:o})){return;
}}if(j){q=s.toggleSelection();}if(p.boundList&&p.boundList.length>0){b.setBoundListProperties(p,n,s.isSelected());
}t=s.effectiveOptions();if(t.includeKeys){m=d.split(t.includeKeys);a.each(m,function(w,v){var u=f.getDataForKey(v.toString());
if(!u.options.isMask){k(u);}});}}k(i);};this.graphics=new b.Graphics(this);};c=b.MapData.prototype;
c.initializeDefaults=function(){this.images=[];this.imageSources=[];this.imageStatus=[];
this.altImagesXref={};this.map=null;this.base_canvas=null;this.overlay_canvas=null;
...
this.currentAreaId=-1;this._tooltip_events=[];this.scaleInfo=null;};c.state=function(){return{complete:this.complete,resizing:this.resizing,zoomed:this.zoomed,zoomedArea:this.zoomedArea,scaleInfo:this.scaleInfo};
...
};c.getSelected=function(){var e="";a.each(this.data,function(g,f){if(f.isSelected()){e+=(e?",":"")+f.key;
...
}if(p){e.attr("data-mapster-key",u);}}m={display:"block",position:"relative",padding:0,width:C.width,height:C.height};
...
for(f=this.keys.length-1;f>=0;f--){e=g.getDataForKey(this.keys[f]);d.updateProps(j,e.effectiveRenderOptions(h),e.options["render_"+h],{alt_image:this.owner.altImage(h)});
}return j;};}(jQuery));(function(a){a.mapster.utils.areaCorners=function(f,q,i){var n,o,l,m,d,b,e,c,g,h,p,k;
n=o=d=e=999999;l=m=b=c=-1;for(k=f.length-2;k>=0;k-=2){g=parseInt(f[k],10);h=parseInt(f[k+1],10);
if(g<n){n=g;c=h;}if(g>l){l=g;e=h;}if(h<o){o=h;b=g;}if(h>m){m=h;d=g;}}if(q&&i){a([[b-q,o-i],[d,o-i],[n-q,c-i],[n-q,e],[c-i,l],[e,l],[b-q,m],[d,m]]).each(function(r,j){if(j[0]>0&&j[1]>0){p=j;
...
a.extend(b.area_defaults,{toolTip:null});b.MapData.prototype.clearTooltip=function(){if(this.activeToolTip){this.activeToolTip.remove();
...
a(i.image).after(l);i.activeToolTip=l;i.activeToolTipID=this.areaId;f=c.areaCorners(g,l.outerWidth(true),l.outerHeight(true));
h=f.tt[0];n=f.tt[1];m={left:h+"px",top:n+"px"};if(!l.css("z-index")||l.css("z-index")==="auto"){m["z-index"]="2000";
}l.css(m).addClass("mapster_tooltip");i.bindTooltipClose("area-click","click",a(i.map));
i.bindTooltipClose("tooltip-click","click",l);if(i.options.toolTipFade){c.setOpacity(l[0],0);
l.show();c.fader(l[0],0,1,j.fadeDuration);}else{l.show();}c.ifFunction(d.onShowToolTip,this.area||null,{toolTip:l,areaOptions:j,key:this.key,selected:this.isSelected()});
