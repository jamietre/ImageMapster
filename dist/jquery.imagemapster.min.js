...
}else{r.addClass(l.wrapClass);}}}k.wrapper=r;k.scaleInfo=o=d.scaleMap(k.images[0],k.images[1],l.scaleMap);
e=k.graphics.createVisibleCanvas(k);m=k.graphics.createVisibleCanvas(k);k.base_canvas=e;
k.overlay_canvas=m;j=a(k.images[0]).clone().addClass("mapster_el").attr({id:null,usemap:null});
a.each(["width","height","padding","border","margin"],function(t,s){j.css(s,h.css(s));
...
};b.utils.scaleMap=function(e,f,g){function h(i){var j={width:d.imgWidth(i,true),height:d.imgHeight(i,true)};
...
if(a.inArray(i,this.options.toolTipClose)>=0){h.unbind(f).bind(f,function(j){if(!d||d(j)){g.clearTooltip();
...
h.bindTooltipClose("tooltip-click","click",l);h.bindTooltipClose("image-mouseout","mouseout",a(h.image),function(n){return(n.relatedTarget.nodeName!=="area");
