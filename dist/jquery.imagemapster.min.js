...
}h.ids.push(e);h[e]=h[g];}return g;},bind:function(h){var f=this,g,i=f.bindTries,e=function(){var j;
j=f.length;while(j-->0){if(!f.isLoaded(j)){break;}}if(f.complete()){f.resolve();}else{if(i-->0){f.imgTimeout=window.setTimeout(function(){e.call(f,true);
},50);}else{f.imageLoadError.call(f);}}};g=f.deferred=d.defer();e();return g;},resolve:function(){var e=this,f=e.deferred;
if(f){e.deferred=null;f.resolve();}},imageLoaded:function(f){var h=this,g=h.indexOf(f.target);
if(g>=0){h.status[g]=true;if(a.inArray(false,h.status)<0){h.resolve();}}},imageLoadError:function(f){clearTimeout(this.imgTimeout);
