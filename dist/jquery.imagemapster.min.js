...
this.initializeDefaults();this.configureOptions(g);this.mousedown=function(i){i.preventDefault();
if(!a.mapster.hasCanvas){this.blur();}};this.mouseover=function(k){var j=f.getAllDataForArea(this),i=j.length?j[0]:null;
...
a(f).attr("src",k);}else{m(f);}};c.bindImages=function(g,e){var h,j,l=this,k=true,m=l.options,n=function(){l.bindImages.call(l,false,e);
},f=function(i){window.clearTimeout(l.imgTimeout);l.imageLoadError(i);};if(g){l.complete=false;
l.triesLeft=l.bindTries;l.imagesLoaded=false;if(l.images.length>2){l.images=l.images.slice(0,2);
l.imageSources=l.imageSources.slice(0,2);l.imageStatus=l.imageStatus.slice(0,2);l.altImagesXref={};
}l.altImagesXref={};if(l.images.length===0){l.addImage(l.image);l.addImage(null,l.image.src);
}if(a.mapster.hasCanvas){l.addImage(null,m.render_highlight.altImage||m.altImage,"highlight");
l.addImage(null,m.render_select.altImage||m.altImage,"select");}}if(l.imagesLoaded){return;
}h=l.images.length;while(h-->0){j=l.images[h];if(!d.isImageLoaded(j)){k=false;if(g){j.onload=n;
j.onerror=f;break;}}}l.imagesLoaded=k;if(l.isReadyToBind()){if(e){e();}else{l.initialize();
}return;}if(l.triesLeft-->0){this.imgTimeout=window.setTimeout(n,50);}else{f();}};
