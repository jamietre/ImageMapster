...
o.map=n;o.bindImages(true);}});};f.init=function(l){var k,j;c.hasCanvas=(document.namespaces&&document.namespaces.g_vml_)?false:a("<canvas></canvas>")[0].getContext?true:false;
...
return e;},_isReadyToBind:function(){return this.complete&&(!this.options.safeLoad||c.windowLoaded);
},_add:function(e){var f=b.push.call(this,e)-1;this.status[f]=false;return f;},indexOf:function(e){return a.inArray(e,this);
},clear:function(){if(this.ids&&this.ids.length>0){a.each(this.ids,function(g,f){delete this[f];
});}this.ids=[];this.length=0;this.status=[];this.splice(0);this.complete=false;this.bindTries=this.options.configTimeout/200;
...
g=h._add(f[0]);f.bind("ferror",function(j){h.imageLoadError.call(h,j);});f.attr("src",i);
}else{g=h._add(a(f)[0]);}if(e){if(this[e]){throw (e+" is already used or is not available as an altImage alias.");
}h.ids.push(e);h[e]=h[g];}return g;},bind:function(h){var g=this,f=d.defer(),i=g.bindTries,e=function(){var j,k=true;
if(!g.complete){j=g.length;while(j-->0){if(!g.isLoaded(j)){k=false;break;}}g.complete=k;
if(g._isReadyToBind()){f.resolve();}else{if(i-->0){g.imgTimeout=window.setTimeout(function(){e.call(g,true);
},50);}else{g.imageLoadError.call(g);}}}};e();return f.promise;},imageLoadError:function(f){clearTimeout(this.imgTimeout);
...
}});});}f.images.bind().then(function(){f.initialize();});};c.configureOptions=function(e){this.area_options=d.updateProps({},b.area_defaults,e);
