...
}var c,d,h=a(),f=e.split(",");g.boundList.each(function(k,j){for(c=0;c<f.length;c++){d=f[c];
if(a(j).is("["+g.listKey+'="'+d+'"]')){h=h.add(j);}}});return h;},setBoundListProperties:function(c,e,d){e.each(function(g,f){if(c.listSelectedClass){if(d){a(f).addClass(c.listSelectedClass);
...
f(j,k.areas);i.updateProps(j.area_options,j.options);a.each(j.data,function(n,m){m._effectiveOptions=null;
...
};e.deselect=function(){e.set.call(this,false);};e.set=function(q,l,r){var n,p,o,k,m,j=[];
function s(t){if(t){switch(q){case true:t.addSelection();break;case false:t.removeSelection(true);
break;default:t.toggleSelection();break;}}}k=i.isBool(r)?r:true;this.each(function(v,u){var t;
o=d.getMapData(u);if(!o){return true;}m="";if(a(u).is("img")){if(d.queueCommand(o,a(u),"set",[q,l,k])){return true;
}if(l instanceof Array){if(l.length){m=l.join(",");}}else{m=l;}if(m){a.each(i.split(m),function(w,x){s(o.getDataForKey(x.toString()));
});if(!q){o.removeSelectionFinish();}}}else{p=a(u).parent()[0];if(n&&p!==n){o=d.getMapData(u);
if(!o){return true;}n=p;}n=p;if(d.queueCommand(o,a(u),"set",[q,l,k])){return true;
}t=o.getDataForArea(u);if(a.inArray(t,j)<0){j.push(t);m+=m===""?"":","+t.key;}}});
a.each(j,function(u,t){s(t);});if(k&&o.options.boundList){d.setBoundListProperties(o.options,d.getBoundList(o.options,m),q);
}return this;};e.unbind=function(j){return(new d.Method(this,function(){this.clearEvents();
...
},callHooks:function(d,c){a.each(this.hooks[d]||[],function(g,f){f.apply(c)
})
...
},setOpacity:function(c,f){if(!a.mapster.hasCanvas){var d=a(c);
d.children().add(d).each(function(h,g){g.style.filter="Alpha(opacity="+String(f*100)+");"
})
...
a.each(Array.prototype.slice.call(arguments,1),function(h,j){a.each(j||{},function(k){if(!e||a.inArray(k,e)>=0){var i=j[k];
if(a.isPlainObject(i)){f[k]=a.extend(f[k]||{},i)
}else{if(i&&i.constructor===Array){f[k]=i.slice(0)
}else{if(typeof i!=="undefined"){f[k]=j[k]
}}}}})
});
return f
...
g.boundList.each(function(k,j){for(c=0;
c<f.length;
c++){d=f[c];
if(a(j).is("["+g.listKey+'="'+d+'"]')){h=h.add(j)
}}});
return h
},setBoundListProperties:function(c,e,d){e.each(function(g,f){if(c.listSelectedClass){if(d){a(f).addClass(c.listSelectedClass)
}else{a(f).removeClass(c.listSelectedClass)
}}if(c.listSelectedAttribute){a(f).attr(c.listSelectedAttribute,d)
}})
...
}}}a(d).each(function(m,l){j=h.func_area.apply(l,h.args)
});
...
this.each(function(v,u){var t;
o=d.getMapData(u);
if(!o){return true
}m="";
if(a(u).is("img")){if(d.queueCommand(o,a(u),"set",[q,l,k])){return true
}if(l instanceof Array){if(l.length){m=l.join(",")
}}else{m=l
}if(m){a.each(i.split(m),function(w,x){s(o.getDataForKey(x.toString()))
});
if(!q){o.removeSelectionFinish()
}}}else{p=a(u).parent()[0];
if(n&&p!==n){o=d.getMapData(u);
if(!o){return true
}n=p
}n=p;
if(d.queueCommand(o,a(u),"set",[q,l,k])){return true
}t=o.getDataForArea(u);
...
a.each(this.data,function(m,l){l.options={}
})
}g(this,j);
...
a(this).each(function(m,l){if(l.nodeName==="IMG"){j=d.getMapData(l);
if(j){k=j.state()
}return false
}});
return k
...
return this.each(function(m,l){var n,o,q,p;
n=a(l);
n.css("border",0);
p=d.getMapData(l);
if(p){e.unbind.apply(n);
if(!p.complete){n.bind();
return true
}p=null
}q=this.getAttribute("usemap");
o=q&&a('map[name="'+q.substr(1)+'"]');
if(!(n.is("img")&&q&&o.size()>0)){return true
}if(!p){p=new d.MapData(this,k);
p.index=c(p);
p.map=o;
p.addImage(this);
p.addImage(null,this.src);
>>>>>>> 3f8fd7a8805465bfc1917bfffaf3576d42bae442
...
f.hasCanvas=false;f.active=false;f.canvas=null;f.width=0;f.height=0;f.shapes=[];f.masks=[];
f.map_data=e;};c=b.Graphics.prototype;c.begin=function(f,g){var e=a(f);this.elementName=g;
this.canvas=f;this.width=e.width();this.height=e.height();this.shapes=[];this.masks=[];
this.active=true;};c.addShape=function(f,g){var e=g.isMask?this.masks:this.shapes;
...
i._addShapeGroupImpl(m,j);});}i._addShapeGroupImpl(e,j);i.render();if(l.fade){d.fader(f,0,(i.hasCanvas?1:l.fillOpacity),l.fadeDuration);
}};b.initGraphics=function(e){c.hasCanvas=e;if(e){c.hex_to_decimal=function(f){return Math.max(0,Math.min(parseInt(f,16),255));
...
};c.renderShape=function(g,j,k){var h,f=j.coords(null,k);switch(j.shape){case"rect":g.rect(f[0],f[1],f[2]-f[0],f[3]-f[1]);
...
break;}};c.addAltImage=function(f,g,h,i){f.beginPath();this.renderShape(f,h);f.closePath();
f.clip();f.globalAlpha=i.altImageOpacity;f.drawImage(g,0,0,h.owner.scaleInfo.width,h.owner.scaleInfo.height);
...
if(g){h=j.createCanvasFor(j.canvas);i=h.getContext("2d");i.clearRect(0,0,h.width,h.height);
...
}else{if(n.options.fill){l.beginPath();j.renderShape(l,n.mapArea);l.closePath();l.fillStyle=j.css3color(n.options.fillColor,n.options.fillOpacity);
...
f.drawImage(h,0,0);}else{f.drawImage(k,0,0);}j.active=false;return j.canvas;};c.createCanvasFor=function(i){var g=a(i),k=g.width()||g[0].width,j=g.height()||g[0].height,f=a('<canvas width="'+k+'" height="'+j+'"></canvas>')[0];
return f;};c.clearHighlight=function(){var f=this.map_data.overlay_canvas;f.getContext("2d").clearRect(0,0,f.width,f.height);
};c.removeSelections=function(){};c.refreshSelections=function(){var f,g=this.map_data;
f=g.base_canvas;g.base_canvas=this.createVisibleCanvas(g.image);a(g.base_canvas).hide();
...
break;}h=a(p);a(l.canvas).append(h);return h;};c.render=function(){var g,f=this;a.each(this.shapes,function(j,h){f.renderShape(h.mapArea,h.options);
...
};c.createCanvasFor=function(g){var f=a(g),j=f.width(),i=f.height();return a('<var width="'+j+'" height="'+i+'" style="zoom:1;overflow:hidden;display:block;width:'+j+"px;height:"+i+'px;"></var>')[0];
...
};}};b.initGraphics(b.hasCanvas);}(jQuery));(function(a){var c,b=a.mapster,d=b.utils;
b.MapData=function(e,g){var f=this;function h(l,i,j){function k(m){if(f.currentAreaId!==m&&f.highlightId>=0){j();
}}if(f.activeAreaEvent){window.clearTimeout(f.activeAreaEvent);f.activeAreaEvent=0;
}if(l<0){return;}if(i.owner.resizing||l){f.activeAreaEvent=window.setTimeout((function(){return function(){h(0,i,j);
...
this.mousedown=function(i){i.preventDefault();};this.mouseover=function(k){var j=f.getAllDataForArea(this),i=j.length?j[0]:null,l;
if(!i||i.isNotRendered()||i.owner.resizing){return;}l=i.effectiveOptions();if(!a.mapster.hasCanvas){this.blur();
}if(f.currentAreaId===i.areaId){return;}if(f.highlightId!==i.areaId){f.clearEffects();
i.highlight(!l.highlight);if(f.options.showToolTip){a.each(j,function(n,m){if(m.effectiveOptions().toolTip){m.showTooltip();
...
}});return e;};c.getAllDataForArea=function(f,g){var h,e,l,k=this,j=a(f).attr(this.options.mapKey);
...
}};c.setHighlightId=function(e){this.highlightId=e;};c.clearSelections=function(){this.graphics.removeSelections();
a.each(this.data,function(g,f){f.selected=false;});};c.setAreaOptions=function(f){var j,g,e,h=f||{};
...
function f(j,H){var i=new b.AreaData(y,j,H);i.areaId=y._xref[j]=y.data.push(i)-1;
return i.areaId;}if(y.complete){return;}y.complete=true;s=a(y.image);B=s.parent().attr("id");
if(B&&B.length>=12&&B.substring(0,12)==="mapster_wrap"){G=s.parent();G.attr("id",y.wrapId());
}else{G=a('<div id="'+y.wrapId()+'"></div>');if(z.wrapClass){if(z.wrapClass===true){G.addClass(s.attr("class"));
}else{G.addClass(z.wrapClass);}}}y.wrapper=G;l=y.graphics.createVisibleCanvas(y.image);
A=y.graphics.createVisibleCanvas(y.image);y.base_canvas=l;y.overlay_canvas=A;y._xref={};
y.data=[];p=!z.mapKey;if(p){z.mapKey="data-mapster-key";}D=(a.browser.msie&&a.browser.version<=7)?"area":(p?"area[coords]":"area["+z.mapKey+"]");
k=a(y.map).find(D);d.scaleMap(y.image,z.scaleMap,function(i){C=y.scaleInfo=i;for(r=k.length-1;
r>=0;r--){h=0;g=k[r];e=a(g);if(!g.coords){continue;}n=p?"":g.getAttribute(z.mapKey);
w=new b.MapArea(y,g,p||!n?"":n);v=w.keys;y.mapAreas.push(w);x=y.mapAreas.length-1;
for(t=v.length-1;t>=0;t--){u=v[t];if(z.mapValue){q=e.attr(z.mapValue);}if(p){h=f(y.data.length,q);
o=y.data[h];o.key=u=h.toString();}else{h=y._xref[u];if(h>=0){o=y.data[h];if(q&&!y.data[h].value){o.value=q;
}}else{h=f(u,q);o=y.data[h];}}w.areaDataXref.push(h);o.areasXref.push(x);}if(!w.nohref){e.bind("mouseover.mapster",y.mouseover).bind("mouseout.mapster",y.mouseout).bind("click.mapster",y.click).bind("mousedown.mapster",y.mousedown);
}if(p){e.attr("data-mapster-key",u);w.keys=[u];}}m={display:"block",position:"relative",padding:0,width:C.width,height:C.height};
if(z.wrapCss){a.extend(m,z.wrapCss);}if(s.parent()[0]!==y.wrapper[0]){s.before(y.wrapper);
}G.css(m);for(r=1;r<y.images.length;r++){G.append(y.images[r]);}a(y.images.slice(1)).hide();
s.css(b.canvas_style);y.images[1].style.cssText=y.image.style.cssText;G.append(y.images[1]).append(l).append(A).append(s);
d.setOpacity(y.image,0);d.setOpacity(y.images[1],1);y.setAreaOptions(z.areas);if(z.isSelectable&&z.onGetList){F=y.data.slice(0);
if(z.sortList){if(z.sortList==="desc"){E=function(j,H){return j===H?0:(j>H?-1:1);
};}else{E=function(j,H){return j===H?0:(j<H?-1:1);};}F.sort(function(j,H){j=j.value;
H=H.value;return E(j,H);});}y.options.boundList=z.onGetList.call(y.image,F);}y.redrawSelections();
if(y.commands.length){a.each(y.commands,function(H,j){b.impl[j.command].apply(j.that,j.args);
});y.commands=[];}if(z.onConfigured&&typeof z.onConfigured==="function"){z.onConfigured.call(s,true);
...
};c.clearMapData=function(f){var e=this;this._clearCanvases(f);a.each(this.data,function(h,g){g.reset(f);
...
b.AreaData=function(f,e,g){this.owner=f;this.key=e||"";this.areaId=-1;this.value=g||"";
this.options={};this.selected=null;this.areasXref=[];this.area=null;};c=b.AreaData.prototype;
c.areas=function(){var e,f=[];for(e=0;e<this.areasXref.length;e++){f.push(this.owner.mapAreas[this.areasXref[e]]);
}return f;};c.coords=function(g,f){var e=[];a.each(this.areas(),function(j,h){e=e.concat(h.coords(g,f));
});return e;};c.reset=function(e){a.each(this.areas(),function(g,f){f.reset(e);});
...
e.selected=this.isSelected();return e;};c.effectiveRenderOptions=function(f,h){var e=this.effectiveOptions(h),g=d.updateProps({},e,e["render_"+f],{alt_image:this.owner.altImage(f)});
return g;};c.changeState=function(f,e){if(a.isFunction(this.owner.options.onStateChange)){this.owner.options.onStateChange.call(this.owner.image,{key:this.key,state:f,selected:e});
...
this.owner.graphics.removeSelections(this.areaId);if(!e){this.owner.removeSelectionFinish();
...
g.keys=d.split(f);};b.MapArea.prototype.coords=function(){return this.originalCoords;
};b.MapArea.prototype.effectiveRenderOptions=function(j,g){var f,e,h=this.owner,k=d.updateProps({},h.area_options);
for(f=this.keys.length-1;f>=0;f--){e=h.getDataForKey(this.keys[f]);d.updateProps(k,e.effectiveRenderOptions(j),e.options["render_"+j],{alt_image:this.owner.altImage(j)});
}return k;};}(jQuery));(function(a){a.mapster.utils.areaCorners=function(f,q,i){var n,o,l,m,d,b,e,c,g,h,p,k;
n=o=d=e=999999;l=m=b=c=-1;for(k=f.length-2;k>=0;k-=2){g=parseInt(f[k],10);h=parseInt(f[k+1],10);
if(g<n){n=g;c=h;}if(g>l){l=g;e=h;}if(h<o){o=h;b=g;}if(h>m){m=h;d=g;}}if(q&&i){a.each([[b-q,o-i],[d,o-i],[n-q,c-i],[n-q,e],[c-i,l],[e,l],[b-q,m],[d,m]],function(r,j){if(j[0]>0&&j[1]>0){p=j;
return false;}});}return{tl:[n,o],br:[l,m],tt:p};};}(jQuery));(function(a){var b=a.mapster,d=b.utils,c=b.MapArea.prototype;
b.utils.getScaleInfo=function(h,g,i,e){var f;if(!i&&!e){f=1;}else{f=i/h||e/g;if(f>0.98&&f<1.02){f=1;
}}return{scale:(f!==1),scalePct:f,realWidth:h,realHeight:g,width:i,height:e,ratio:i/e};
};b.utils.scaleMap=function(h,n,e){var j,l,m,o,g,i=a(h),k=this;if(!i.length){return;
}o=i.width();g=i.height();function f(){if(!l){l=this.height;m=this.width;e(k.getScaleInfo(m,l,o,g));
}}if(n){j=new Image();j.onload=f;j.src=h.src;if(j.width&&j.height){f.call(j);}}else{l=g;
m=o;}};b.MapData.prototype.resize=function(n,l,f){var j,p,r,i,e,o={},m,g,k=this;if(typeof n==="object"){o=n;
}else{o.width=n;o.height=l;o.duration=f;}r=o.width;i=o.height;e=o.duration||1000;
if(k.scaleInfo.width===r&&k.scaleInfo.height===i){return;}j=k.highlightId;function q(s,u,t){if(a.mapster.hasCanvas){s.width=u;
s.height=t;}else{a(s).width(u);a(s).height(t);}}function h(){q(k.overlay_canvas,r,i);
if(o.highlight){if(j>=0){var s=k.data[j];s.tempOptions={fade:false};k.getDataForKey(s.key).highlight();
s.tempOptions=null;}}q(k.base_canvas,r,i);k.redrawSelections();k.resizing=false;if(a.isFunction(o.callback)){o.callback();
...
a.each(k.data,function(t,s){a.each(s.areas(),function(v,u){u.resize();});});};b.MapArea=d.subclass(b.MapArea,function(){if(this.owner.scaleInfo.scale){this.resize();
}});c.coords=function(k,e){var f,g=[],i=k||this.owner.scaleInfo.scalePct,h=e||0;if(i===1&&e===0){return this.originalCoords;
}for(f=0;f<this.length;f++){g.push(Math.round(this.originalCoords[f]*i)+h);}return g;
};c.resize=function(){this.area.coords=this.coords().join(",");};c.reset=function(){this.area.coords=this.coords(1).join(",");
...
i.render();
if(l.fade){d.fader(f,0,(i.hasCanvas?1:l.fillOpacity),l.fadeDuration)
}};
b.initGraphics=function(e){c.hasCanvas=e;
if(e){c.hex_to_decimal=function(f){return Math.max(0,Math.min(parseInt(f,16),255))
};
c.css3color=function(f,g){return"rgba("+this.hex_to_decimal(f.substr(0,2))+","+this.hex_to_decimal(f.substr(2,2))+","+this.hex_to_decimal(f.substr(4,2))+","+g+")"
};
...
function h(l,i,j){function k(m){if(f.currentAreaId!==m&&f.highlightId>=0){j()
}}if(f.activeAreaEvent){window.clearTimeout(f.activeAreaEvent);
f.activeAreaEvent=0
}if(l<0){return
}if(i.owner.resizing||l){f.activeAreaEvent=window.setTimeout((function(){return function(){h(0,i,j)
}
}(i)),l||100)
}else{k(i.areaId)
}}this.index=-1;
...
this.mousedown=function(i){i.preventDefault()
};
this.mouseover=function(k){var j=f.getAllDataForArea(this),i=j.length?j[0]:null,l;
if(!i||i.isNotRendered()||i.owner.resizing){return
}l=i.effectiveOptions();
if(!a.mapster.hasCanvas){this.blur()
}if(f.currentAreaId===i.areaId){return
}if(f.highlightId!==i.areaId){f.clearEffects();
i.highlight(!l.highlight);
if(f.options.showToolTip){a.each(j,function(n,m){if(m.effectiveOptions().toolTip){m.showTooltip()
}})
}}f.currentAreaId=i.areaId;
if(a.isFunction(f.options.onMouseover)){f.options.onMouseover.call(this,{e:k,options:l,key:i.key,selected:i.isSelected()})
}};
this.mouseout=function(j){var k,l,i=f.getDataForArea(this),m=f.options;
if(f.currentAreaId<0||!i){return
}l=f.getDataForArea(j.relatedTarget);
if(l===i){return
}f.currentAreaId=-1;
i.area=null;
h(m.mouseoutDelay,i,f.clearEffects);
if(a.isFunction(m.onMouseout)){m.onMouseout.call(this,{e:j,options:m,key:k,selected:i.isSelected()})
}};
this.clearEffects=function(){var i=f.options;
f.ensureNoHighlight();
if(i.toolTipClose&&a.inArray("area-mouseout",i.toolTipClose)>=0&&f.activeToolTip){f.clearTooltip()
}};
this.click=function(m){var r,n,o,p,j,k,s,t=this,i=f.getDataForArea(this),q=f.options;
function l(u){var v;
j=(u.isSelectable()&&(u.isDeselectable()||!u.isSelected()));
if(j){p=!u.isSelected()
}else{p=u.isSelected()
}o=b.getBoundList(q,u.key);
if(a.isFunction(q.onClick)){k=q.onClick.call(t,{e:m,listTarget:o,key:u.key,selected:p});
if(d.isBool(k)){if(!k){return false
}s=a(u.area).attr("href");
if(s!=="#"){window.location.href=s;
return false
}}}if(j){r=u.toggleSelection()
}if(q.boundList&&q.boundList.length>0){b.setBoundListProperties(q,o,u.isSelected())
}v=u.effectiveOptions();
if(v.includeKeys){n=d.split(v.includeKeys);
a.each(n,function(y,x){var w=f.getDataForKey(x.toString());
if(!w.options.isMask){l(w)
}})
}}m.preventDefault();
if(i&&!i.owner.resizing){if(!a.mapster.hasCanvas){this.blur()
}q=f.options;
l(i)
}};
...
a.each(this.data,function(g,f){if(f.isSelected()){e+=(e?",":"")+this.key
}});
return e
};
c.getAllDataForArea=function(f,g){var h,e,l,k=this,j=a(f).attr(this.options.mapKey);
if(j){l=[];
j=d.split(j);
for(h=0;
h<(g||j.length);
h++){e=k.data[k._idFromKey(j[h])];
e.area=f.length?f[0]:f;
l.push(e)
}}return l
};
c.getDataForArea=function(f){var e=this.getAllDataForArea(f,1);
return e?e[0]||null:null
};
...
if(this.highlightId>=0){this.graphics.clearHighlight();
e=this.data[this.highlightId];
e.changeState("highlight",false);
this.setHighlightId(-1)
}};
c.setHighlightId=function(e){this.highlightId=e
};
...
c.isNotRendered=function(){var e=a(this.area);
...
f>=0;
f--){e=h.getDataForKey(this.keys[f]);
d.updateProps(k,e.effectiveRenderOptions(j),e.options["render_"+j],{alt_image:this.owner.altImage(j)})
}return k
}
}(jQuery));
...
d=g
}}if(q&&i){a.each([[b-q,o-i],[d,o-i],[n-q,c-i],[n-q,e],[c-i,l],[e,l],[b-q,m],[d,m]],function(r,j){if(j[0]>0&&j[1]>0){p=j;
return false
}})
...
});this._tooltip_events.push({object:g,event:e});}};b.AreaData.prototype.showTooltip=function(){var l,h,n,m,f,g,e,j=this.effectiveOptions(),i=this.owner,d=i.options,k=i.options.toolTipContainer;
i.cancelClear=true;if(i.activeToolTipID===this.areaId){return;}if(typeof k==="string"){e=a(k);
}else{e=a(k).clone();}l=e.html(j.toolTip).hide();if(this.area){g=c.split(this.area.coords,",");
}else{g=[];a.each(this.areas(),function(p,o){g=g.concat(o.coords());});}i.clearTooltip();
a(i.image).after(l);i.activeToolTip=l;i.activeToolTipID=this.areaId;c.setOpacity(l[0],0);
l.show();f=c.areaCorners(g,l.outerWidth(true),l.outerHeight(true));h=f.tt[0];n=f.tt[1];
...
}else{c.setOpacity(l[0],1);}c.ifFunction(d.onShowToolTip,this.area||null,{toolTip:l,areaOptions:j,key:this.key,selected:this.isSelected()});
