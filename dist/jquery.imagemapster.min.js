...
}},isImageLoaded:function(c){var d;if(typeof c.complete!=="undefined"&&!c.complete){return false;
}if(typeof c.naturalWidth!=="undefined"&&(c.naturalWidth===0||c.naturalHeight===0)){return false;
}d=a(c);return !!(d.width()&&d.height());},fader:(function(){var c={},e=0,d=function(g,k,h,f){var i,j,l=a.mapster.utils;
...
};e.set=function(r,m,q){var o,p,n,k;function s(t){if(t){switch(r){case true:t.addSelection(q);
break;case false:t.removeSelection(true);break;default:t.toggleSelection(q);break;
}}}function j(t){if(t&&a.inArray(t,k)<0){k.push(t);n+=(n===""?"":",")+t.key;}}function l(t){a.each(k,function(v,u){s(u);
});if(!r){t.removeSelectionFinish();}if(t.options.boundList){d.setBoundListProperties(t.options,d.getBoundList(t.options,n),r);
}}this.each(function(u,t){var v;p=d.getMapData(t);if(p!==o){if(o){l(o);}k=[];n="";
}if(p){v="";if(a(t).is("img")){if(!d.queueCommand(p,a(t),"set",[r,m,q])){if(m instanceof Array){if(m.length){v=m.join(",");
}}else{v=m;}if(v){a.each(i.split(v),function(w,x){j(p.getDataForKey(x.toString()));
o=p;});}}}else{if(!d.queueCommand(p,a(t),"set",[r,m,q])){j(p.getDataForArea(t));o=p;
}}}});if(p){l(p);}return this;};e.unbind=function(j){return(new d.Method(this,function(){this.clearEvents();
...
k=a(A.map).find(F);A.scaleInfo=E=d.scaleMap(t,A.images[1],B.scaleMap);u=a(A.images[0]).clone().addClass("mapster_el").attr({id:null,usemap:null});
...
};b.utils.scaleMap=function(e,f,g){function h(i){var j=a(i),k={width:j.width(),height:j.height()};
if(!(k.width&&k.height)){throw ("Another script, such as an extension, appears to be interfering with image loading. Please let us know about this.");
}return k;}return this.getScaleInfo(h(f),h(e));};b.MapData.prototype.resize=function(n,l,f){var j,p,r,i,e,o={},m,g,k=this;
