...
},split:function(g,d){var f,e,c=g.split(",");
for(f=c.length-1;
f>=0;
f--){e=a.trim(c[f]);
c[f]=d?d(e):e;
if(!c[f]){c=c.splice(f,1)
}}return c
...
}var c,d,h=a(),f=a.mapster.utils.split(e);
...
e.set=function(r,m,s){var o,q,p,l,n,k=[];
function t(u){if(u){switch(r){case true:u.addSelection();
break;
case false:u.removeSelection(true);
break;
default:u.toggleSelection();
break
}}}function j(u){if(a.inArray(u,k)<0){k.push(u);
n+=(n===""?"":",")+u.key
}}l=i.isBool(s)?s:true;
this.each(function(v,u){var w;
p=d.getMapData(u);
if(!p){return true
}w="";
if(a(u).is("img")){if(d.queueCommand(p,a(u),"set",[r,m,l])){return true
}if(m instanceof Array){if(m.length){w=m.join(",")
}}else{w=m
}if(w){a.each(i.split(w),function(x,y){j(p.getDataForKey(y.toString()))
})
}}else{q=a(u).parent()[0];
if(o&&q!==o){p=d.getMapData(u);
if(!p){return true
}o=q
}o=q;
if(d.queueCommand(p,a(u),"set",[r,m,l])){return true
}j(p.getDataForArea(u))
}});
a.each(k,function(v,u){t(u)
});
if(!r){p.removeSelectionFinish()
}if(l&&p.options.boundList){d.setBoundListProperties(p.options,d.getBoundList(p.options,n),r)
}return this
};
...
p.addImage(null,k.render_select.altImage||k.altImage,"select")
}p.bindImages()
}})
};
...
a(d.map_cache).each(function(n,m){if(!m.complete&&m.isReadyToBind()){m.initialize()
}})
})
};
...
i.render();
if(l.fade){d.fader(f,0,(b.hasCanvas?1:l.fillOpacity),l.fadeDuration)
}};
...
e.closePath();
e.clip();
e.globalAlpha=h.altImageOpacity||h.fillOpacity;
...
c.clearSelections=function(){a.each(this.data,function(g,f){if(f.selected){f.removeSelection(true)
}});
...
k=a(y.map).find(D);
y.scaleInfo=C=d.scaleMap(s,y.images[1],z.scaleMap);
for(r=k.length-1;
r>=0;
r--){h=0;
g=k[r];
...
}else{E=function(i,j){return i===j?0:(i<j?-1:1)
}
}F.sort(function(i,j){i=i.value;
j=j.value;
return E(i,j)
})
...
a.each(this.data,function(h,g){g.reset()
});
this.data=null;
...
c.coords=function(f){var e=[];
a.each(this.areas(),function(h,g){e=e.concat(g.coords(f))
});
return e
};
c.reset=function(){a.each(this.areas(),function(g,f){f.reset()
});
this.areasXref=[];
this.options=null
};
...
c.effectiveRenderOptions=function(f,h){var e,g=this.optsCache;
if(!g||f==="highlight"){e=this.effectiveOptions(h);
g=d.updateProps({},e,e["render_"+f],{alt_image:this.owner.altImage(f)});
if(f!=="highlight"){this.optsCache=g
}}return g
};
...
b.MapArea.prototype.reset=function(){this.area=null
};
b.MapArea.prototype.coords=function(e){return a.map(this.originalCoords,function(f){return e?f:f+e
})
}
}(jQuery));
(function(a){var b=a.mapster.utils;
b.areaCorners=function(e,x,o){var n,t,u,r,s,h,f,i,g,l,m,v,q,p,w,c,d,k=[];
for(q=0;
q<e.length;
q++){d=e[q];
p=b.split(d.coords,parseInt);
switch(d.shape){case"circle":l=p[0];
m=p[1];
w=p[2];
k=[];
for(q=0;
q<360;
q+=20){c=q*Math.PI/180;
k.push(l+w*Math.cos(c),m+w*Math.sin(c))
}break;
case"rect":k.push(p[0],p[1],p[2],p[1],p[2],p[3],p[0],p[3]);
break;
default:k=k.concat(p);
break
}}t=u=h=i=999999;
r=s=f=g=-1;
for(q=k.length-2;
q>=0;
q-=2){l=parseInt(k[q],10);
m=parseInt(k[q+1],10);
if(l<t){t=l;
g=m
}if(l>r){r=l;
...
a.each([[f-x,u-o],[h,u-o],[t-x,g-o],[t-x,i],[g-o,r],[i,r],[f-x,s],[h,s]],function(y,j){if(!n&&(j[0]>0&&j[1]>0)){v=j;
n=true;
return false
}});
if(!n){v=[r,s]
}}return{tl:[t,u],br:[r,s],tt:v}
}
}(jQuery));
...
b.utils.scaleMap=function(f,g,l){var j,k,m,e,h=a(f),i=a(g);
m=h.width();
e=h.height();
k=i.width();
j=i.height();
return this.getScaleInfo(k,j,m,e)
};
...
b.MapArea=d.subclass(b.MapArea,function(){this.base.init();
if(this.owner.scaleInfo.scale){this.resize()
}});
...
b.MapData.prototype.clearTooltip=function(){if(this.activeToolTip){this.activeToolTip.stop().remove();
...
b.AreaData.prototype.showTooltip=function(){var l,h,n,m,g,d,f,j=this.effectiveOptions(),i=this.owner,e=i.options,k=i.options.toolTipContainer;
i.cancelClear=true;
if(i.activeToolTipID===this.areaId){return
}if(typeof k==="string"){f=a(k)
}else{f=a(k).clone()
}l=f.html(j.toolTip).hide();
i.clearTooltip();
a(i.image).after(l);
i.activeToolTip=l;
...
c.setOpacity(l[0],0);
l.show();
d=this.area?[this.area]:a.map(this.areas(),function(o){return o.area
});
g=c.areaCorners(d,l.outerWidth(true),l.outerHeight(true));
h=g.tt[0];
n=g.tt[1];
...
if(i.options.toolTipFade){c.fader(l[0],0,1,j.fadeDuration)
}else{c.setOpacity(l[0],1)
