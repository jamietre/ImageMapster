...
j=f.length;while(j-->0){if(!f.isLoaded(j)){break;}}if(f.complete()){f.resolve();}else{if(h-->0){f.imgTimeout=window.setTimeout(function(){e.call(f,true);
},50);}else{f.imageLoadError.call(f);}}};f.deferred=d.defer();e();return f.deferred.promise;
},resolve:function(){var e=this,f=e.deferred;if(f){e.deferred=null;f.resolve();}},imageLoaded:function(f){var h=this,g=h.indexOf(f.target);
