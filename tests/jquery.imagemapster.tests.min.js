...
v=h.mapster("get_options");v.singleSelect=true;h.mapster("rebind",v);A.assertCsvElementsEq(h.mapster("get"),"TX,AK","Rebind with singleSelect reverted to original state");
h.mapster("set",true,"MI");A.assertEq(h.mapster("get"),"MI","Single select worked.");
h.mapster("set_options",{isDeselectable:false});$('area[state="MI"]').first().click();
...
h.mapster("set_options",{singleSelect:false,isDeselectable:true,areas:[{key:"ME",isDeselectable:false}]});
...
q.mapster("unbind");j.addTest("Mapster Command Queue Tests",function(C){var y,u,z,w=$("#test_elements *").length;
function v(){var E="Master Command Queue (async completion)";var D=$(this);j.addTest(E,function(G){var F;
G.assertCsvElementsEq(D.mapster("get"),z,"Only initial selections present when simulating non-ready image");
F=$("#test_elements *").length;G.assertNotEq(F,w,"Dom size is unequal before unbinding at test end");
D.mapster("unbind");F=$("#test_elements *").length;G.assertEq(F,w,"Dom size is equal at test end");
});}u=false;y=$("#usa_image");y.mapster("test","var m=$.mapster.MapData.prototype; m.old=m.isImageLoaded;m.isImageLoaded=function(){return false;}");
z="AK,TX";var A=$.extend({},i,{onConfigured:v});y.mapster(A);y.mapster("set",true,"KS,KY");
z="AK,KY,TX,KS";var B="Can't access get with non-ready image";try{y.mapster("get");
C.assertIsTruthy(false,B);}catch(x){C.assertIsTruthy(true,B);}y.mapster("test",'if (typeof $.mapster.MapData !== "undefined") {$.mapster.MapData.prototype.isImageLoaded=$.mapster.MapData.prototype.old;}');
